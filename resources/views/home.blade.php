...
@section('content')
<div class="min-h-[calc(100vh-var(--nav-height)-var(--footer-height,0px))] bg-gradient-to-br from-indigo-900 via-blue-800 to-blue-600 text-white">
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="welcome-section mb-10 opacity-0 transform translate-y-10 transition-all duration-1000 ease-out" id="welcomeSection">
            <div class="flex justify-between items-center">
                <div>
                    <h1 class="text-4xl md:text-5xl font-bold mb-2">Welcome, {{ Auth::user()->name }}</h1>
                    <p class="text-xl text-blue-200">Your journey continues today. Let's make it count.</p>
                </div>
                <button id="openWelcomeModal" class="bg-blue-500 hover:bg-blue-600 text-white p-3 px-6 rounded-full transition-colors shadow-lg flex items-center justify-center">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            ...
            <div class="bg-white bg-opacity-10 backdrop-blur-lg rounded-xl p-6 shadow-xl border border-white border-opacity-20 transform hover:scale-105 transition-all duration-300 opacity-0" id="nextTrainingCard">
                ...
                <div class="mb-4" id="nextTrainingContent">
                    @if(isset($nextTrainings) && count($nextTrainings) > 0)
                        @foreach($nextTrainings->take(2) as $training)
                            <div class="mb-4 pb-4 {{ !$loop->last ? 'border-b border-white border-opacity-20' : '' }}">
                                <div class="text-2xl font-bold text-blue-300 mb-2">{{ $training->title }}</div>
                    ...
                    @else
                        <div class="text-center py-6">
                            <p class="text-xl text-blue-200 mb-4">No upcoming objectives scheduled</p>
                        </div>
                    @endif
                </div>
            </div>
            ...
            <div class="bg-white bg-opacity-10 backdrop-blur-lg rounded-xl p-6 shadow-xl border border-white border-opacity-20 transform hover:scale-105 transition-all duration-300 opacity-0 flex flex-col" id="raceCard">
                <h2 class="text-2xl font-bold mb-4 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex-grow mb-4" id="nextRaceContent">
                    @if(isset($nextRace))
                        <div class="text-3xl font-bold text-orange-300 mb-2">{{ $nextRace->title }}</div>
                        <div class="flex items-center mb-2 gap-2">
                            <i class="fa-regular fa-calendar"></i>
                    ...
                    @else
                        <div class="text-center py-6">
                            <p class="text-xl text-blue-200 mb-4">No upcoming races scheduled</p>
                        </div>
                    @endif
                </div>
                ...
                @if(isset($nextRace))
                    <div class="mt-auto pt-4 border-t border-white border-opacity-20">
                        <h3 class="font-semibold mb-2">Time Until Race:</h3>
                        <div class="grid grid-cols-4 gap-2 text-center" id="raceCountdown" data-target-date="{{ $nextRace->date }}">
                            <div class="bg-orange-800 bg-opacity-50 rounded-lg p-2">
                                <span id="raceDays" class="text-3xl font-bold block">--</span>
                                <span class="text-xs text-orange-200">DAYS</span>
                            </div>
                            <div class="bg-orange-800 bg-opacity-50 rounded-lg p-2">
                                <span id="raceHours" class="text-3xl font-bold block">--</span>
                                <span class="text-xs text-orange-200">HRS</span>
                            </div>
                            <div class="bg-orange-800 bg-opacity-50 rounded-lg p-2">
                                <span id="raceMinutes" class="text-3xl font-bold block">--</span>
                                <span class="text-xs text-orange-200">MINS</span>
                            </div>
                            <div class="bg-orange-800 bg-opacity-50 rounded-lg p-2">
                                <span id="raceSeconds" class="text-3xl font-bold block">--</span>
                                <span class="text-xs text-orange-200">SECS</span>
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="bg-white bg-opacity-10 backdrop-blur-lg rounded-xl p-6 shadow-xl border border-white border-opacity-20 transform hover:scale-105 transition-all duration-300 opacity-0 flex flex-col" id="statsCard">
                <h2 class="text-2xl font-bold mb-4 flex items-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="space-y-6 flex-grow mb-4">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium text-blue-200">Weekly Distance</span>
                            @if(isset($weeklyGoal->distance) && $weeklyGoal->distance > 0)
                                <span class="text-sm font-bold">{{ number_format($weeklyStats->distance ?? 0, 1) }} / {{ number_format($weeklyGoal->distance, 1) }} km</span>
                            @else
                                <span class="text-sm font-bold">{{ number_format($weeklyStats->distance ?? 0, 1) }} km</span>
                            @endif
                        </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium text-blue-200">Weekly Duration</span>
                            @if(isset($weeklyGoal->duration) && $weeklyGoal->duration > 0)
                                <span class="text-sm font-bold">{{ formatTime($weeklyStats->duration ?? 0) }} / {{ formatTime($weeklyGoal->duration) }}</span>
                            @else
                                <span class="text-sm font-bold">{{ formatTime($weeklyStats->duration ?? 0) }}</span>
                            @endif
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            @if(isset($weeklyGoal->duration) && $weeklyGoal->duration > 0)
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium text-blue-200">Weekly Elevation</span>
                            @if(isset($weeklyGoal->elevation) && $weeklyGoal->elevation > 0)
                                <span class="text-sm font-bold">{{ number_format($weeklyStats->elevation ?? 0) }} / {{ number_format($weeklyGoal->elevation) }} m</span>
                            @else
                                <span class="text-sm font-bold">{{ number_format($weeklyStats->elevation ?? 0) }} m</span>
                            @endif
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            @if(isset($weeklyGoal->elevation) && $weeklyGoal->elevation > 0)
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium text-blue-200">Monthly Consistency</span>
                            <span class="text-sm font-bold">{{ $monthlyConsistency ?? 0 }}%</span>
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                ...
                @endphp
                <div class="mt-auto pt-4 border-t border-white border-opacity-20">
                    <a href="{{ route('calendar') }}#{{ $currentMonthSlug }}" class="flex items-center justify-center gap-2 px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg transition-colors w-full">
