...
<style>
    ...
    .home-content-container::-webkit-scrollbar {
        width: 0px; /* Masque la scrollbar sur Chrome, Safari et Opera */
        display: none; /* Suppression complète de la scrollbar */
    }
</style>
...
<div class="home-content-container">
    <div class="container mx-auto px-4 py-12">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
            <!-- Next Trainings Card with Animation -->
            <div class="{{ themeClass('card-bg', 'bg-white bg-opacity-10') }} backdrop-blur-lg rounded-xl p-6 shadow-xl border {{ themeClass('card-border', 'border-white border-opacity-20') }} transform hover:scale-105 transition-all duration-300 opacity-0" id="nextTrainingCard">
                <h2 class="text-2xl font-bold mb-4 flex items-center {{ themeClass('text-primary', 'text-white') }}">
                ...
                <div class="mb-4" id="nextTrainingContent">
                    @if(isset($nextTrainings) && count($nextTrainings) > 0)
                        @foreach($nextTrainings->take(2) as $training)
                            <div class="mb-4 pb-4 {{ !$loop->last ? 'border-b border-white border-opacity-20' : '' }}">
                                <div class="text-2xl font-bold {{ themeClass('text-accent', 'text-blue-300') }} mb-2">{{ $training->title }}</div>
                    ...
                    @else
                        <div class="text-center py-6">
                            <p class="text-xl {{ themeClass('text-secondary', 'text-blue-200') }} mb-4">No upcoming objectives scheduled</p>
                        </div>
                    @endif
                </div>
            </div>
            
            <!-- Race Card with Animation -->
            <div class="{{ themeClass('card-bg', 'bg-white bg-opacity-10') }} backdrop-blur-lg rounded-xl p-6 shadow-xl border {{ themeClass('card-border', 'border-white border-opacity-20') }} transform hover:scale-105 transition-all duration-300 opacity-0 flex flex-col" id="raceCard">
                <h2 class="text-2xl font-bold mb-4 flex items-center {{ themeClass('text-primary', 'text-white') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="flex-grow mb-4" id="nextRaceContent">
                    @if(isset($nextRace))
                        <div class="text-3xl font-bold {{ themeClass('text-accent', 'text-orange-300') }} mb-2">{{ $nextRace->title }}</div>
                        <div class="flex items-center mb-2 gap-2 {{ themeClass('text-primary', 'text-white') }}">
                            <i class="fas fa-calendar"></i>
                    ...
                    @else
                        <div class="text-center py-6">
                            <p class="text-xl {{ themeClass('text-secondary', 'text-blue-200') }} mb-4">No upcoming races scheduled</p>
                        </div>
                    @endif
                </div>
                ...
                @if(isset($nextRace))
                    <div class="mt-auto pt-4 border-t {{ themeClass('border-divider', 'border-white border-opacity-20') }}">
                        ...
                        <div class="grid grid-cols-4 gap-2 text-center" id="raceCountdown" data-target-date="{{ $nextRace->date }}">
                            <div class="{{ themeClass('countdown-bg', 'bg-orange-800 bg-opacity-50') }} rounded-lg p-2">
                                <span id="raceDays" class="text-3xl font-bold block {{ themeClass('text-primary', 'text-white') }}">--</span>
                                <span class="text-xs {{ themeClass('text-secondary', 'text-orange-200') }}">DAYS</span>
                            </div>
                            <div class="{{ themeClass('countdown-bg', 'bg-orange-800 bg-opacity-50') }} rounded-lg p-2">
                                <span id="raceHours" class="text-3xl font-bold block {{ themeClass('text-primary', 'text-white') }}">--</span>
                                <span class="text-xs {{ themeClass('text-secondary', 'text-orange-200') }}">HRS</span>
                            </div>
                            <div class="{{ themeClass('countdown-bg', 'bg-orange-800 bg-opacity-50') }} rounded-lg p-2">
                                <span id="raceMinutes" class="text-3xl font-bold block {{ themeClass('text-primary', 'text-white') }}">--</span>
                                <span class="text-xs {{ themeClass('text-secondary', 'text-orange-200') }}">MINS</span>
                            </div>
                            <div class="{{ themeClass('countdown-bg', 'bg-orange-800 bg-opacity-50') }} rounded-lg p-2">
                                <span id="raceSeconds" class="text-3xl font-bold block {{ themeClass('text-primary', 'text-white') }}">--</span>
                                <span class="text-xs {{ themeClass('text-secondary', 'text-orange-200') }}">SECS</span>
                            </div>
                        </div>
                    </div>
                @endif
            ...
            <div class="{{ themeClass('card-bg', 'bg-white bg-opacity-10') }} backdrop-blur-lg rounded-xl p-6 shadow-xl border {{ themeClass('card-border', 'border-white border-opacity-20') }} transform hover:scale-105 transition-all duration-300 opacity-0 flex flex-col" id="statsCard">
                <h2 class="text-2xl font-bold mb-4 flex items-center {{ themeClass('text-primary', 'text-white') }}">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 mr-2" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                ...
                <div class="space-y-6 flex-grow mb-4">
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium {{ themeClass('text-secondary', 'text-blue-200') }}">Weekly Distance</span>
                            @if(isset($weeklyGoal->distance) && $weeklyGoal->distance > 0)
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ number_format($weeklyStats->distance ?? 0, 1) }} / {{ number_format($weeklyGoal->distance, 1) }} km</span>
                            @else
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ number_format($weeklyStats->distance ?? 0, 1) }} km</span>
                            @endif
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            ...
                            @else
                                <div class="bg-blue-400 h-2.5 rounded-full progress-bar" style="width: 100%"></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium {{ themeClass('text-secondary', 'text-blue-200') }}">Weekly Duration</span>
                            @if(isset($weeklyGoal->duration) && $weeklyGoal->duration > 0)
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ formatTime($weeklyStats->duration ?? 0) }} / {{ formatTime($weeklyGoal->duration) }}</span>
                            @else
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ formatTime($weeklyStats->duration ?? 0) }}</span>
                            @endif
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            @if(isset($weeklyGoal->duration) && $weeklyGoal->duration > 0)
                            ...
                            @else
                                <div class="bg-green-400 h-2.5 rounded-full progress-bar" style="width: 100%"></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium {{ themeClass('text-secondary', 'text-blue-200') }}">Weekly Elevation</span>
                            @if(isset($weeklyGoal->elevation) && $weeklyGoal->elevation > 0)
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ number_format($weeklyStats->elevation ?? 0) }} / {{ number_format($weeklyGoal->elevation) }} m</span>
                            @else
                                <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ number_format($weeklyStats->elevation ?? 0) }} m</span>
                            @endif
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                            @if(isset($weeklyGoal->elevation) && $weeklyGoal->elevation > 0)
                                <div class="bg-orange-400 h-2.5 rounded-full progress-bar" style="width: {{ isset($weeklyStats->elevation) ? min(($weeklyStats->elevation / $weeklyGoal->elevation) * 100, 100) : 0 }}%"></div>
                            @else
                                <div class="bg-orange-400 h-2.5 rounded-full progress-bar" style="width: 100%"></div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div>
                        <div class="flex justify-between items-center mb-2">
                            <span class="text-sm font-medium {{ themeClass('text-secondary', 'text-blue-200') }}">Monthly Consistency</span>
                            <span class="text-sm font-bold {{ themeClass('text-primary', 'text-white') }}">{{ $monthlyConsistency ?? 0 }}%</span>
                        </div>
                        <div class="w-full bg-gray-700 rounded-full h-2.5">
                ...
                @endphp
                <div class="mt-auto pt-4 border-t {{ themeClass('border-divider', 'border-white border-opacity-20') }}">
                    <a href="{{ route('calendar') }}#{{ $currentMonthSlug }}" class="flex items-center justify-center gap-2 px-4 py-2 {{ themeClass('button-bg', 'bg-blue-500 hover:bg-blue-600') }} {{ themeClass('button-text', 'text-white') }} rounded-lg transition-colors w-full">
                        <i class="fas fa-calendar"></i>
    ...
    <div id="welcomeModal" class="fixed inset-0 bg-gray-900 bg-opacity-75 items-center justify-center z-50 hidden">
        <div class="{{ themeClass('modal-bg', 'bg-white') }} rounded-lg shadow-xl p-8 max-w-5xl w-full mx-auto max-h-[90vh] overflow-y-auto">
            <h2 class="text-2xl text-center font-bold {{ themeClass('modal-text', 'text-gray-800') }} mb-10">Bienvenue sur Zone 2</h2>
            <p class="{{ themeClass('modal-text-secondary', 'text-gray-600') }} mb-4">
            ...
            <div class="mb-4">
                <h3 class="font-semibold {{ themeClass('modal-text', 'text-gray-800') }} mb-2">Pourquoi une approche par blocs ?</h3>
                <p class="{{ themeClass('modal-text-secondary', 'text-gray-600') }} mb-4">
                ...
                <ul class="list-disc pl-6 {{ themeClass('modal-text-secondary', 'text-gray-600') }} space-y-2">
            ...
            <div class="mb-4">
                <h3 class="font-semibold {{ themeClass('modal-text', 'text-gray-800') }} mb-2">Principales fonctionnalités :</h3>
                <ul class="list-disc pl-6 {{ themeClass('modal-text-secondary', 'text-gray-600') }} space-y-2">
            ...
            <div class="mb-4">
                <h3 class="font-semibold {{ themeClass('modal-text', 'text-gray-800') }} mb-2">Exemple d'utilisation :</h3>
                <ul class="list-disc pl-6 {{ themeClass('modal-text-secondary', 'text-gray-600') }} space-y-2">
            ...
            <p class="{{ themeClass('modal-text-secondary', 'text-gray-600') }} mb-4 mt-10">
            ...
            <p class="{{ themeClass('modal-text-secondary', 'text-gray-600') }} mb-2 font-medium">
                Utilisez le formulaire de contact intégré pour nous faire part de vos remarques à tout moment.
            </p>
            <p class="{{ themeClass('modal-text-secondary', 'text-gray-600') }}">
                Merci pour votre participation et votre confiance !<br>
                Laurent
            </p>
            <div class="text-center mt-8">
                <button id="closeWelcomeModal" class="{{ themeClass('button-bg', 'bg-blue-600 hover:bg-blue-700') }} {{ themeClass('button-text', 'text-white') }} py-2 px-6 rounded-lg">
                    J'ai compris
                </button>
            </div>
        </div>
